...
topwhile:
  while (fgets(gamestr, 512, fp) != NULL) {
    ...
    for (int k = 0; k < j; k++) {
      ...
      // for (int l = 0; l < sizeof(colorstr) / sizeof(colorstr[0]); l++) {
      for (int l = 0; l < c; l++) {
        val = atoi(colorstr[l]);
        // printf("%d\n", val);
        ...
        if ((strcmp(components[1], "red") == 0 && val > red) ||
            (strcmp(components[1], "green") == 0 && val > green) ||
            (strcmp(components[1], "blue") == 0 && val > blue)) {
          sum += i;
          // printf("this can't happen %d\n", i);
